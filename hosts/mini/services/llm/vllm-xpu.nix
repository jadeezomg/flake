...
}:
{
  ...
  services.vllm-xpu = {
    ...
    # below (`languageModelOnly = false` + `limitMmPerPrompt`), so the vision tower
    # is loaded and needs the XPU torchvision wheel in the vLLM closure.
    ...
    instances.chat = {
      ...
      # Packs as `auto_round:auto_gptq`, so it loads via the gptq path. AutoRound
      # quantizes only `model.language_model.layers`, so the vision tower stays bf16
      # and is retained — this same checkpoint serves images (see limitMmPerPrompt).
      ...
      kvCacheDtype = "fp8";
      # Image input ON. Qwen3.5 is multimodal; serving it as a VL model means vLLM
      # loads the vision encoder and accepts `image_url` content in /v1/chat/completions
      # (still advertised as the shared `local-chat` id). `limitMmPerPrompt` caps the
      # multimodal budget at one image per request (no video) to bound vision-encoder
      # memory on the ~15 GiB B50. If the vision `profile_run` OOMs / hits the
      # "Chunk prefill kernel not compiled" fallback, see the dedicated note in
      # docs/hosts/mini-vllm-xpu.md (lower maxModelLen/maxNumSeqs, or a custom
      # `withKernelConfig` kernel build).
      ...
      maxModelLen = 131072;
      # Continuous batching. Lowered from 8 to 4 now that vision is on: each
      # concurrent multimodal request also reserves vision-encoder activation +
      # image-embedding budget, which is the scarce resource on the ~15 GiB B50
      # (KV itself stays cheap — int4 weights + fp8 KV give a huge token pool).
      # Raise again once vllm-xpu-chat boots cleanly with images.
      maxNumSeqs = 4;
