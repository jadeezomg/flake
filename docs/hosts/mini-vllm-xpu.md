...
- **VRAM / context:** mini's GPU is an **Intel Arc Pro B50** with only **~15 GiB**. Unquantized **Qwen3.5-9B** (bf16 **~18 GiB** of weights) does **not** fit — it fails vLLM's startup memory pre-check (`Free memory on device xpu:0 … less than desired GPU memory utilization`) regardless of `gpuMemoryUtilization`, and lowering that knob only moves the failure to an OOM during weight load. Chat therefore runs **[Intel/Qwen3.5-9B-int4-AutoRound](https://huggingface.co/Intel/Qwen3.5-9B-int4-AutoRound)** (int4 AutoRound, **~5 GiB** weights — same pattern as the reference brutus `Intel/…-int4-AutoRound`). It packs as `auto_round:auto_gptq`, so it loads via **`quantization = "gptq"`**. Other settings: **`pkgs.vllm-xpu-unstable.withTorchvision true`** (Qwen3.5 imports Qwen VL image-processing code during vLLM inspection — and now genuinely needs it, vision is enabled), **`languageModelOnly = false`** (vision enabled — see § *Image input* below), **`limitMmPerPrompt = { image = 1; video = 0; }`**, **`maxNumSeqs = 4`**, **`enforceEager = true`**, pinned **`--revision`**. Embedding is disabled so chat gets the whole budget.

  Qwen3.5 is a **hybrid linear-attention** model: only **8 of 32** layers are full attention (`full_attention_interval = 4`); the rest keep a fixed-size recurrent state that does **not** grow with context. With 4 GQA KV heads × 256 head-dim that is just **16 KiB/token at fp8** (32 KiB bf16), so KV is cheap and context is **not** the constraint here. With **`kvCacheDtype = "fp8"`** the model's full **262,144** context costs only **~4 GiB** of KV. Current config: **`kvCacheDtype = "fp8"`**, **`maxModelLen = 131072`** (128k; raise toward 262144 once vLLM's reported `GPU KV cache size` confirms room), **`gpuMemoryUtilization = 0.9`**. The B50 is **headless** (display runs on the Iris Xe iGPU), so vLLM can claim more of the card than the reference's 0.85 — but **0.95** failed the startup free-memory pre-check (needed 15.13 of 15.01 GiB free), so 0.90 is the safe ceiling. The ~1.5 GiB runtime overhead is vLLM's Level-Zero/oneCCL context + activation peak, **not** a desktop compositor, so headless does not reduce it. If fp8 KV destabilises boot (see the FP8 troubleshooting note below), fall back to **`kvCacheDtype = null`** (bf16 KV still fits ~120k tokens). **Image input is now enabled on the SAME checkpoint** — see § *Image input* below.

## Image input

Mini now runs the chat model **with vision enabled** — **no repo swap**, the SAME **[Intel/Qwen3.5-9B-int4-AutoRound](https://huggingface.co/Intel/Qwen3.5-9B-int4-AutoRound)** serves images. It is natively multimodal (`Qwen3_5ForConditionalGeneration`, `image_token_id` set); **AutoRound quantizes ONLY `model.language_model.layers`**, so the **vision tower stays bf16** and is retained in the int4 checkpoint — the same int4 weights serve text and images. Config in `hosts/mini/services/llm/vllm-xpu.nix`:

- **`languageModelOnly = false`** (was `true` — vision encoder profiling now runs)
- **`limitMmPerPrompt = { image = 1; video = 0; }`**
- **`maxNumSeqs`** lowered to **4** (from 8) to leave vision-encoder activation / image-embedding headroom on the ~15 GiB Arc Pro B50

Images go in standard OpenAI **`image_url`** content parts to **`/v1/chat/completions`**; the served id stays **`local-chat`** (curl example in **`docs/hosts/mini-llm-hosting.md`** § *APIs — chat with image*).

**This is the live, risky config.** Enabling multimodal on the Intel **XPU** path is exactly the scenario in § *Chunk prefill kernel not compiled / EngineCore failed to start / XPU OOM during vision `profile_run`* below — consult it if **`vllm-xpu-chat`** fails to boot or enters a restart loop once images are served. Text-only fallback: set **`languageModelOnly = true`** (and you may raise **`maxNumSeqs`** back to 8). The reliable, low-risk alternative for image input is the **llama.cpp** backend (Vulkan, `--mmproj-auto`) — see `docs/hosts/mini-llm-hosting.md`.
...
- **`local-chat`** — **[unsloth/gemma-4-12B-it-qat-GGUF](https://huggingface.co/unsloth/gemma-4-12B-it-qat-GGUF)** (`UD-Q4_K_XL`, Google's **QAT** Gemma 4 12B). The one repo bundles QAT weights + the **MTP** drafter + the **`mmproj-*.gguf`** vision projector. Chat runs with **vision** (`mmproj-auto`) **on**. **MTP speculative decoding** (`spec-type = draft-mtp`, `spec-draft-n-max = 2`, ~1.5–2.2× faster generation) is **wired but currently disabled** (`chatMtp = false`) — the deployed llama.cpp **build 9503** has the `--spec-type draft-mtp` flag but **not** the `gemma4-assistant` draft-architecture loader, so the MTP drafter is not loaded — see **`docs/hosts/mini-llm-hosting.md`** § *Change GGUF models / MTP tuning*. Chat context is **64K total** (32K per conversation, `cache-type-k/v = q8_0`), **trimmed from 96K** to co-reside with the embedder.
...
4. **Gemma 4 without vLLM on XPU:** switch to the llama.cpp backend (**`miniLlmBackend = "llamacpp"`**) to serve **[unsloth/gemma-4-12B-it-qat-GGUF](https://huggingface.co/unsloth/gemma-4-12B-it-qat-GGUF)** (`UD-Q4_K_XL` QAT + vision; **MTP** speculative decoding is wired but currently disabled, `chatMtp = false`) as **`local-chat`** via **`llama-cpp-gemma`** on the shared **port 8000** (Vulkan path). That backend runs in **router mode** and additionally serves **`local-embed`** (F2LLM-v2-0.6B embeddings) on the same port — see § *llama.cpp GGUF (the other backend)* above. This is a backend swap, not an additional server — vLLM is no longer imported while llama.cpp is the active backend.
...
**Cause:** vLLM imports `vllm.model_executor.models.qwen3_5`, which imports `qwen3_vl`; Transformers then imports `torchvision.transforms.v2`. This bites regardless of vision config — even the **text-only fallback** (**`languageModelOnly = true`**) hits it during inspection. The service fails before binding **`:8000`**.

**Fix:** keep chat on **`pkgs.vllm-xpu-unstable.withTorchvision true`**. This adds the XPU torchvision wheel to the vLLM Python closure (required for the now-enabled vision path, and also for inspection in the text-only fallback). With vision enabled (**`languageModelOnly = false`**, the live config) torchvision is genuinely exercised for image preprocessing.
...
**This is now the live config**, not a hypothetical — mini serves images on the vLLM backend (**`languageModelOnly = false`**, see § *Image input* above), so this is the section to consult if **`vllm-xpu-chat`** fails to boot or restart-loops once images are in play.

**Cause:** **`vllm-xpu-kernels`** ships a **fixed set** of chunk-prefill shapes. With the **default** `pkgs.vllm-xpu-unstable` package (no custom **`withKernelConfig`**), **multimodal** startup profiling (now active on `Intel/Qwen3.5-9B-int4-AutoRound`, same class as **Qwen3-VL**) can request a shape **not** in that set, so vLLM **falls back** to a PyTorch attention path that can **explode memory** on a **~16 GiB** dGPU.
