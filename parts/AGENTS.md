...
- Workaround overlays guard themselves with `lib/expiry.nix` (bound per file in `parts/overlays/default.nix`); guard attribute values, never the attrset an overlay returns (infinite recursion).
